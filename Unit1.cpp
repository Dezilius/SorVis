...
Timer2->Interval = ScrollBar1->Position;
     //for (int k = 1; k < num; k *= 2)                        // Tried to iterate one comparison at a time
     //{
         for (int left = 0; left < num; left += k*2)           // Comparing all pairs, fours etc... at a time
         {
             rght = left + k;
             rend = rght + k;
             ...
             while (i < rght && j < rend)
             {
                   if (arr[i] < arr[j])                        // If value from the arr[i] 2-4-8... element array is larger
                   {                                           // copy it to arrTemp
                      arrTemp[m] = arr[i];
                      i++;
                   }
                   ...
                   {
                      arrTemp[m] = arr[j];                     // as above but with arr[j] ...
                      j++;
                   }
                   m++;
             }
             while (i < rght)                                  // If there is anything left in the left array copy it
             {
                   arrTemp[m] = arr[i];
                   i++;
                   m++;
             }
             while (j <= rend)                                 // If there is anything left in the right array copy it
             {
                   arrTemp[m] = arr[j];
                   j++;
                   m++;
             }
             for (m = left; m < rend; m++)                     // Actualize main array, which is displayed on the screen
                 {
                 arr[m] = arrTemp[m];
